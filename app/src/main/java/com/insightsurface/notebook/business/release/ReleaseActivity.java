...
public class ReleaseActivity extends BaseActivity {
    ...
    private void doUploadRelease() {
        ...
        if (TextUtils.isEmpty(noteId)) {
            ...
            note.put("title", titleEt.getText().toString().replaceAll(" ", "<space/>").replaceAll("\n", "<br/>"));// 如果点了赞就是 1，而点了不喜欢则为 -1，没有做任何操作就是默认的 0
            note.put("content", contentEt.getText().toString().replaceAll(" ", "<space/>").replaceAll("\n", "<br/>"));// 留言的内容
